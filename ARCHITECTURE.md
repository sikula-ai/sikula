...
An interrupted `moving` record instead uses `cleanup --quarantine-only --force`: this
removes only retained quarantine storage, preserves the child worktree and task state,
marks semantic and validation gates stale, and leaves the failed state available for an
explicit `run --task-id ... --reset-failed` resume.
