...
snapshot to `state.run_invocation_records` before pipeline work starts. New
states carry `run_invocation_schema_version = 1`, which proves that this audit
history is complete from state creation. Legacy states keep a null marker even
when a newer Sikula version later appends resume records, so downstream metrics
cannot mistake partial invocation history for complete configuration evidence.
Terminal no-op `run --task-id` calls do not append records.
...
| `run_invocation_schema_version` | `int \| None` | `StateStore.create()` / `cmd_review()` | `1` for states whose run-invocation audit is complete from creation. `None` identifies legacy states for which later resume records cannot reconstruct earlier invocations. This marker is audit-only and never drives pipeline behavior. |
| `run_invocation_records` | `list[dict]` | Orchestrator | Append-only record for each non-terminal `Orchestrator.run()` call, containing `started_at` and the effective `config_snapshot` used by that invocation. It preserves configuration changes across resume for audit and external metrics projection. Terminal no-op calls are omitted, and records never drive pipeline behavior. |
