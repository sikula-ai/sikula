...
### Fixed
- **Antigravity Read-Only Enforcement**: Antigravity calls now deliver complete requests through bounded, Git-excluded runtime files instead of the ignored stdin sentinel, with stale transport cleanup on run start, resume, and worktree finalization. Generation, analyst, reviewer, and security-reviewer calls reject active hooks before writing those files, disable slash-command expansion, and use a generated read-tool-only agent. Repository read-only calls retain disposable-copy mutation detection and treat an attempted project mutation as one non-retryable provider contract violation while preserving its usage observation.
