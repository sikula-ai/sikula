...
- Failed validation records now include high-signal diagnostic summary lines, and task terminal summaries show those lines for recovered build/test/check failures so successful self-healed runs still reveal the concrete compiler error, failed test, assertion, or linter rule that was repaired.
