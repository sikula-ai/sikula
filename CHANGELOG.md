...
- Validation command extraction now only treats explicit validation contexts as commands, so prose that happens to start with a known tool name does not trigger pre-agent validation coverage failures.
- Validation command coverage treats Gradle/Maven wrapper spelling and npm/pnpm/Yarn `test` script shortcuts as equivalent for otherwise identical commands, while same-tool-family commands with materially different flags, targets, scripts, packages, schemes, or paths remain uncovered; near matches are included only as diagnostic context before agents run.
