...
### Fixed
- **Antigravity Read-Only Enforcement**: Antigravity calls now deliver complete requests through bounded temporary task files instead of the ignored stdin sentinel. Analyst, reviewer, and security-reviewer calls also reject active hooks, disable slash-command expansion, use a generated read-tool-only agent, retain disposable-copy mutation detection, and treat an attempted project mutation as one non-retryable provider contract violation while preserving its usage observation.
