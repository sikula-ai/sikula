...
### Added
- **PR-Ready Task Summary**: Added `sikula summary <task-id>` to render deterministic, privacy-safe PR Markdown from a completed isolated task or review-fix with a publishable commit, without modifying state or exposing raw task text, prompts, provider output, review prose, diagnostics, or absolute paths.
- **Antigravity Usage Metrics**: Antigravity calls now consume the CLI's structured JSON result and contribute explicit input, output, total, and cache-read token counts to provider-neutral usage records. The minimum supported Antigravity CLI version is now 1.1.12.
...
- **Delivery Recovery And Amendments**: Delivery units can resume, reconcile terminal children, and retry linked failed children through `run-next --reset-failed`. Model-assisted `delivery amend prepare` plus deterministic `amend apply` preview/application safely split eligible units without losing completed work or audit history; amendment preflight now uses the assembled delivery head and apply retains the updated plan and replacement contracts in `final_branch` without changing the operator checkout.
...
- **Delivery Boundary Stops**: Delivery children now inherit fingerprinted unit constraints, enforce effective production scope across provider attempts and retained deterministic tool mutations, and stop with stable non-retryable scope-amendment or external-dependency dispositions. Malformed disposition advertisements fail closed, while run-next and bounded-run dry-runs mirror scope preflight. Status, execution, and amendment preparation preserve sanitized evidence and expose explicit recovery actions without leaking raw child state.
- **Delivery Source Continuity**: Delivery preparation and amendments now preserve canonical source-task assets through deterministic unit assignment and rendering while independently verifying inherited hard constraints and rejecting verbatim source-task constraint summaries before publishing executable units or projecting checked plans.
...
### Changed
- **Delivery Planner Budgets**: Delivery child planners now receive the unit's effective step limit and re-evaluate an oversized plan once, while preserving the existing pre-implementation split gate when safe consolidation is not possible.
...
### Fixed
- **Delivery Scope Audit Completeness**: Write-agent audits now reject tracked provider-config collisions, fail safely on ordinary workspace-setup errors, inspect every physical provider attempt before retries or later Fixer calls, persist the invocation's immutable lexical and resolved production and active Fixer test-write roots independently of progress heartbeats, recover interrupted writes before runtime-scope changes, traverse directories through no-follow identity-checked handles, inspect the full worktree for nested project roots, accept unchanged internal symlink scope aliases while rejecting retargets and nested escapes, retain only bounded mixed-source content, and preserve successful-audit and separately labeled sibling paths for amendment evidence.
- **Delivery Boundary Consistency**: Assembled dependencies can no longer broaden an exact-file unit scope by deleting it or replacing it with a directory, child constraint contexts retain every bounded legacy-valid unit identity without leaking unsafe values into provider prompts, the highest-priority persisted terminal failure code controls amendment recovery and parent projection over older disposition or wrapper-failure evidence, and direct run/status output no longer recommends reset for non-retryable delivery children.
- **Delivery Boundary Recovery**: Delivery scope is revalidated in the active assembled child worktree before runtime construction, malformed advertised Implementer dispositions persist a non-retryable terminal stop even after partial writes, amendments reassign constrained superseded units to every replacement before plan validation, and amendment evidence plus child recovery consistently use the injected state store.
- **Delivery Boundary Validation**: Fresh plans independently verify source-task constraint completeness, every constrained amendment receives and verifies inherited rules, production-scope audits include gitignored filesystem writes and interruption baselines, invalid assembled scopes stop terminally, and amendment evidence reports the actual runtime-narrowed scope.
- **Antigravity Read-Only Enforcement**: Antigravity calls now deliver complete requests through a separate temporary attached workspace instead of the ignored stdin sentinel, keeping prompt files outside project snapshots and result commits. All calls reject active hooks before writing those files; generation, analyst, reviewer, and security-reviewer calls also disable slash-command expansion and use a generated read-tool-only agent. Repository read-only calls retain disposable-copy mutation detection and treat an attempted project mutation as one non-retryable provider contract violation while preserving its usage observation.
- **Delivery Amendment Proposal Replay:** Prepared split proposals with multiple unit budget fields now preserve canonical budget ordering across JSON storage and replay, so an immediate dry-run or apply no longer fails with `delivery_amend.proposal_output_mismatch` despite unchanged inputs.
