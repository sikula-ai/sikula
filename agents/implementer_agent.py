...
- If the task or implementation prompt contains an `Asset manifest`, treat it as
  part of the delivery contract. Use delivery assets only within the requested
  scope, do not copy reference-only assets into production files, and do not
  invent missing provenance, license, or target information. When a delivery
  asset has no requested target, choose the project-conventional location from
  the codebase and keep the change minimal.
