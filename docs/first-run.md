...
Default Sikula runs create a git worktree from `HEAD`. Config and guideline files must be tracked and clean, otherwise the task worktree cannot see them. Review worktrees start from the reviewed branch or captured start commit, so that branch must also contain any configured guidelines or rule files used by review.
