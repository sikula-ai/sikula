...
| Antigravity | Requires 1.1.12; rejects active hooks, uses plan mode plus a generated read-tool-only agent, and rejects any disposable-copy change. | Rejects external symlinks on kept paths; runs `agy` on the task worktree with a workspace instruction. |

For Antigravity, Sikula blocks read-only calls when the provider reports any enabled workspace, plugin, or global hook because hook commands are outside the plan-mode tool boundary. Read-only calls omit automatic permission approval and disable shell, inherited MCP, subagent, skill dependency, and plugin dependency capabilities in a generated custom agent. Sikula still protects the task checkout with a disposable copy, kept-path symlink validation, and a full retained-path snapshot; a detected change is a non-retryable provider contract violation. Write-capable behavior is unchanged. OS-level prevention of writes outside these workspaces depends on Antigravity CLI sandbox behavior.
